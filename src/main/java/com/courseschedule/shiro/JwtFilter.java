...
/**
 * 继承shiro内置的AuthenticatingFilter，一个内置了可以自动登录方法的过滤器。<br />
 * 重写了以下四个方法
 */
@Component
public class JwtFilter extends AuthenticatingFilter {
    ...
    protected boolean onAccessDenied(ServletRequest servletRequest, ServletResponse servletResponse) throws Exception {
        ...
        if (StringUtils.isEmpty(token)) {
            ...
            // 需要注意的是，这里Filter在全局异常处理的更外层
            // 所以全局异常处理无法拦截这里抛出的new ExpiredCredentialsException("token已失效，请重新登录！");
            // 如需处理得另外想办法
            // 要不等会把有效期改成forever算了...
        }
        // 执行自动登录
     ...
     * (重写了onLoginFailure方法以规范返回给前端的格式)
     *
     * @param token
     * @param e
     * @param request
